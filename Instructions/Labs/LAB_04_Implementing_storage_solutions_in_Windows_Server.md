...
> **Note**: You need to revert the virtual machines (VM) between each exercise. Because most of the VMs are Windows Server 2019 Server Core VMs, the time to revert and restart is faster than attempting to undo changes made to the storage environment in the exercises.
...
1. On **SEA-ADM1**, in Server Manager, add the **Data Deduplication** role to **SEA-SVR3**.
1. Share the C:\Labfiles folder.
...
1. Exit Windows PowerShell and map drive **X** to **\\\\SEA-ADM1\\Labfiles**, amd then navigate to X:, cd Mod04
...
1. Notice that **M:\Data** has free space.
...
2. Execute the following command to start Data Deduplication process, and then select Enter:

   ```powershell
   Start-DedupJob m: -Type Optimization –Memory 50
   ```

3. Switch to the **File Explorer** window, and then navigate to **E:\Labfiles\Mod02\Data**.
4. Observe the **Size** and **Size on disk** property values for the **report.xlsx** file.
5. Wait for five to ten minutes to allow the deduplication job to run.
6. Switch back to the Windows PowerShell window.
...
8. In Server Manager, select **File and Storage Services**, select Disk **1**, and then select Volume **M**.
...
> **Note**: Word wrap is used to display the previous command. Do not use word wrap when entering the command in Windows PowerShell.
...
1. On **SEA-ADM1**, in the **Server Manager** window, in **File and Storage Services**, under **Disks**, select the **SEA-DC1** server. Note it only contains the boot and system volume drive C.
...
2. Notice the two new 5-gigabyte (GB) disks on the **SEA-DC1** server that are offline. Notice that the bus enter is **iSCSI**. (If you're in the **File and Storage Services** section of **Server Manager**, you might need to select the refresh button to bring up the two new disks.)
...
To meet some of the requirements for high availability, you decided to evaluate redundancy options in Storage Spaces. Additionally, you want to test the provisioning of new disks to the storage pool.

The main tasks for this exercise are:

1. Create a storage pool by using the iSCSI disks attached to **SEA-SVR3**.
2. Create a three-way mirrored disk on **SEA-SVR3**.
3. Copy a file to the volume on the three-way mirror, and verify it is present in File Explorer.
4. Disconnect the disk and verify file availability.
5. Add a new disk to storage pool.

> **Note**: In **Windows Server 2019**, you cannot disconnect a disk in a storage pool. You can only remove it. Also, you cannot remove a disk from a three-way mirror without adding a new disk first.
...
### Task 3: Copy a file to the volume, and verify it is present in File Explorer
...
2. Switch back to **SEA-ADM1**.
3. Create a new folder named **Test Data**, and then create a new document named **Document1.txt** in that folder.
...
2. Remove the top disk in the list from the storage pool.
...
3. Open **Document1.txt**, add some additional text, and then save it.
...
2. Open Windows PowerShell ISE and load the **C:\Labfiles\Mod04\Implement-StorageSpacesDirect.ps1** script.
...
3. Run the commands in Step 1. This command installs the Failover Clustering role service on **SEA-SVR1**, **SEA-SVR2** and **SEA-SVR3**. The second command restarts the three servers, which is required to complete the install, and the third command installs the Failover Cluster Manager console on **SEA-ADM1**.  
...
3. Return to the **Failover Cluster Manager**, and observe the **Cluster Pool 1** object in **Pools**.
4. Return to **Windows PowerShell ISE**, and run the step 6 command, which creates the **CSV** file system.
5. Return to the **Failover Cluster Manager**, and observe the **Cluster Virtual Disk (CSV)** object in **Disks**.
...
2. Return to the **Failover Cluster Manager**, and observe the **S2D-SOFS** object in **Roles**.
...
4. Return to the **Failover Cluster Manager**, and observe the **VM01** object in **Shares**.

### Task 5: Verify Storage Spaces Direct functionality

1. On **SEA-ADM1**, in **File Explorer**, create a folder named **VMFolder**.
...
4. Return to the **Failover Cluster Manager**, and observe the **Cluster Virtual Disk (CSV)** information in the **Disks** node. (Notice the **Health Status** is set to **Warning**, and the **Operational Status** is **Degraded**.)
